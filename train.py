...
def main():
    ...
    parser.add_argument('--caption_vector_length', type=int, default=512, help='Caption Vector Length')
...
def save_for_vis(data_dir, real_images, generated_images, image_files):
    ...
    for i in range(0, real_images.shape[0]):
        real_image_255 = (real_images[i] * 255).astype(np.uint8)  # Scale and convert to uint8
        imageio.imwrite(join(data_dir, 'samples/{}_{}.jpg'.format(i, image_files[i].split('/')[-1])), real_image_255)

        fake_image_255 = (generated_images[i] * 255).astype(np.uint8)  # Scale and convert to uint8
        imageio.imwrite(join(data_dir, 'samples/fake_image_{}.jpg'.format(i)), fake_image_255)
