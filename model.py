...
class GAN:
    ...
    def build_model(self):
        ...
        g_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(disc_fake_image_logits, tf.ones_like(disc_fake_image)))

        d_loss1 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(disc_real_image_logits, tf.ones_like(disc_real_image)))
        d_loss2 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(disc_wrong_image_logits, tf.zeros_like(disc_wrong_image)))
    ...
    def sampler(self, t_z, t_text_embedding):
        ...
        reduced_text_embedding = ops.lrelu(ops.linear(t_text_embedding, self.options['z_dim'], 'g_embedding'))
    ...
    def discriminator(self, image, t_text_embedding, reuse=False):
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            ...
            tiled_embeddings_adjusted = ops.conv2d(tiled_embeddings, self.options['df_dim'] * 8, k_h=1, k_w=1, d_h=1, d_w=1,
                                                   name='embedding_conv_adjust')
            h3_concat = tf.concat([h3, tiled_embeddings_adjusted], axis=3, name='h3_concat')
            h3_new = ops.lrelu(
